...
<template>
  <div class="form-container">
    ...
    <p v-if="errors?.address_region" class="error">{{ errors.address_region }}</p>
    <input v-model="employeeData.address_settlement" placeholder="Населённый пункт" required/>
    <p v-if="errors?.address_settlement" class="error">{{ errors.address_settlement }}</p>
    <input v-model="employeeData.address_street" placeholder="Улица" required/>
    <p v-if="errors?.address_street" class="error">{{ errors.address_street }}</p>
    ...
    <p v-if="errors?.address_building" class="error">{{ errors.address_building }}</p>
    <input v-model="employeeData.address_apartment" placeholder="Квартира"/>
    <p v-if="errors?.address_apartment" class="error">{{ errors.address_apartment }}</p>
    <h5>Паспортные данные</h5>
    <input v-model="employeeData.passport_series" placeholder="Серия" required/>
    <p v-if="errors?.passport_series" class="error">{{ errors.passport_series }}</p>
    <input v-model="employeeData.passport_number" placeholder="Номер" required/>
    <p v-if="errors?.passport_number" class="error">{{ errors.passport_number }}</p>
    <input type="date" v-model="employeeData.passport_issued_date" required/>
    <p v-if="errors?.passport_issued_date" class="error">{{ errors.passport_issued_date }}</p>
    <input v-model="employeeData.passport_unit_code" placeholder="Код подразделения" required/>
    <p v-if="errors?.passport_unit_code" class="error">{{ errors.passport_unit_code }}</p>
    <input v-model="employeeData.passport_issued_by" placeholder="Кем выдан" required/>
    <p v-if="errors?.passport_issued_by" class="error">{{ errors.passport_issued_by }}</p>
