...
exports.getDepartmentsNames = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Ошибка при получении списка отделов.", error });
  }
};
exports.getOrganizationsNames = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Ошибка при получении списка организаций.", error });
  }
};
exports.getActionsNames = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Ошибка при получении списка действий.", error });
  }
};
exports.getEmployeesWithDepartmentsAndPositions = async (req, res) => {
  try {
    const employeesWithDepartmentsAndPositions =
      await DropdownListOptions.getEmployeesWithDepartmentAndPosition();
    res.status(200).json({ employeesWithDepartmentsAndPositions });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Ошибка при получении списка сотрудников.", error });
  }
};
