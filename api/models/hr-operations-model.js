...
class HrOperationsModel {
  static async getAll() {
    const result = await client.query(`
            SELECT ho.id,
                   ...
                   ho.created_at
            FROM employees e
                     LEFT JOIN hr_operations ho ON e.id = ho.employee_id
  ...
  static async findByEmployee(id) {
    const result = await client.query(
      "SELECT * FROM hr_operations WHERE employee_id = $1",
      [id],
    );
    return result.rows;
  }
  ...
  static async create(data) {
    const result = await client.query(
      `INSERT INTO hr_operations
             (employee_id, action_id, salary, position_id, department_id, created_at)
